...
def ytsearch(casca, trigger):
    """Search YouTube"""
    #modified from ytinfo: Copyright 2010-2011, Michael Yanovich, yanovich.net, Kenneth Sham.
    if not hasattr(casca.config, 'google_dev_apikey'):
        return casca.say('Please sign up for a Google Developer API key to use this function.')
    key = casca.config.google_dev_apikey

    query = trigger.group(2).encode('utf-8').strip()
    #uri = BASE_URL + "search?part=snippet&type=video&q=" + query + "&key=" + key
    uri = BASE_URL + "search?part=snippet&type=video&q={}&key={}".format(query,key) 
    ...
    casca.say(return_text + all_entries)


def youtube_search(casca, input):
    origterm = input.groups()[1]
    if not origterm:
        return casca.say('Perhaps you meant ".yt pugs"?')

    error = None

    try:
        ytsearch(casca, input)
    except IOError:
...
def ytinfo(casca, input):
    video_entry = ytget(casca, input)
    ...
    casca.say(entry_text)


def youtube_info(casca, input):
    origterm = input.groups()[1]
    if not origterm:
        return casca.say('Perhaps you meant ".youtube_info pzPxhaYQQK8"?')
...
def title(casca, match):
    ...
    casca.say(HTMLParser().unescape(message))

    return True


def ytget(casca, trigger):
    if not hasattr(casca.config, 'google_dev_apikey'):
        return 'err'
    ...
    try:
        ...
        uri = BASE_URL + "videos?part=snippet,contentDetails,statistics&id={}&key={}".format(vid_id,key)
        bytes = web.get(uri)
        result = json.loads(bytes)
        video_entry = result['items'][0]
    except IndexError:
        casca.say('Video not found through the YouTube API.')
        return 'err'
    except Exception:
        casca.say('Something went wrong when accessing the YouTube API.')
        traceback.print_exc()
        return 'err'
