...
def man(casca, input):
    ...
    term = input.groups()[1]
    if not term:
        return casca.say('Perhaps you meant ".man ls"?')

    if len(term.split(' ')) > 1:
        return casca.say('I was expecting only one search term')
    ...
    for h in h2s:
        if n in h:
            return casca.say("Couldn't find the man page {0} on man.he.net".format(term))

    lines = soup.pre.contents[0]
    try:
        desc_idx = lines.index('DESCRIPTION')
    except:
        return casca.say('Could not find a "DESCRIPTION" section. :(')

    try:
        desc_end_idx = lines[desc_idx:].index('\n\n')
    except:
        return casca.say('Could not find an ending to the "DESCRIPTION" section. :-(')

    description = lines[desc_idx:desc_idx + desc_end_idx]

    casca.say(man_uri % t)
    casca.say("I am sending you the content of this page in a private message")
    description = description.replace('DESCRIPTION\n', 'DESCRIPTION of %s\n' % (term))

    for c in description.split('\n'):
        casca.msg(input.nick, c, False, False, 1)
