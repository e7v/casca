#!/usr/bin/env python
"""
unostats.py -- casca's uno stat generator
...
def showstats (casca, input):
    ...
    if c.isdigit():
        ...
        for z in prescores[:c]:
            casca.msg(input.nick, STRINGS['SCORE_ROW'] % (i, z[0], z[3], z[1], z[2], float(z[3])/float(z[1]), float(z[2])/float(z[1])*100, ( float(z[3]) / int(z[1]) ) / ( 1.01 - ( float(z[2]) / int(z[1]) ) )))
        ...
        for y in prescores:
            if y[0] == t:
                casca.msg(input.nick, STRINGS['SCORE_ROW'] % (j, y[0], y[3], y[1], y[2], float(y[3])/float(y[1]), float(y[2])/float(y[1])*100, ( float(y[3]) / int(y[1]) ) / ( 1.01 - ( float(y[2]) / int(y[1]) ) )))
            j += 1
