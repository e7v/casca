#!/usr/bin/env python
"""
remind.py - casca Reminder Module
...
def remind(casca, input):
    ...
    if duration >= 60:
        try:
            ...
            casca.reply('Please enter a more realistic time-frame.')
...
def at(casca, input):
    ...
    if not m:
        ...
        return casca.reply("Sorry, I couldn't find the time. " + help_txt)
    ...
    if not m:
        return casca.reply("Sorry, I couldn't figure out what date you wanted. " + help_txt)
    ...
    if duration >= (86400.0 * 365 * 2):  # 2 years
        ...
        return casca.reply('Sorry, but that occurs in the past! Please select a time in the future.')

    ## who do we need to remind? and where? and what?
    reminder = (input.sender, input.nick, bytes)

    ## store such information
    try: casca.rdb[unix_stamp_event].append(reminder)
    except KeyError: casca.rdb[unix_stamp_event] = [reminder]
    ...
    casca.reply('Reminding at %s %s - in %s %s' % (t, z, t_duration, phrase))
at.commands = ['at']
