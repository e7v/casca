#!/usr/bin/env python
"""
southpark.py - casca Southpark Module
...
def southpark (casca, input):
    ...
    if len(text) > 1:
        if text[1] == 'cleartimes':
            cache['TIMES'] = None
            cachets['TIMES'] = None
            casca.reply("Southpark times successfully cleared.")
            return
        elif text[1] == 'clearnewep':
            cache['NEW-EPI'] = None
            cachets['NEW-EPI'] = None
            casca.reply("New episodes cleared from cache.")
        elif text[1] == 'times':
            southparktimes(casca,input)
            return
    else:
        getNewShowDate(casca)
southpark.commands = ['southpark']
southpark.priority = 'low'
southpark.rate = 30

def getNewShowDate (casca):
    global cache, cachets
    tsnow = datetime.now()
...
def southparktimes (casca, input):
...
def printListings (casca):
    for i in cache['TIMES']:
        casca.say('%s:   #%s - \x02%s\x02 %s (%s)   %s' % (time.strftime('%a %b %d', i[0]), i[1]+' '*(maxepilen-len(i[1])), i[2], ' '*(maxtitlelen-len(i[2])+5) , i[3], 'Comedy Central'))
