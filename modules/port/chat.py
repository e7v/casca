...
def chat(casca, input):
    ...
    if channel.startswith('+#') or channel.startswith('@#'):
        ...
        ## in a channel and prepended with casca's name
        pm = False
        ...
        ## in a PM and not prepended with casca's name
        pm = True
        if text.startswith('.') or (hasattr(casca.config, 'prefix') and text.startswith(casca.config.prefix)):
            return
        elif text.startswith(casca.config.nick + ':'):
    ...
    if msgo:
        ...
        response = re.sub('(?i)clever(me|script|bot)', 'casca', msgo)
        ...
        if pm:
            ...
            if hasattr(casca.config, 'logchan_pm'):
                beginning = ':%s PRIVMSG %s :' % (casca.config.nick, input.sender)
                casca.msg(casca.config.logchan_pm, beginning + response)
        else:
            delim = random.choice((',', ':'))
            msg = '%s' % (response)
