#!/usr/bin/env python
'''
warnings.py -- casca NWS Alert Module
...
def nws_lookup(casca, input):
    ''' Look up weather watches, warnings, and advisories. '''
    text = input.group(2)
    if not text:
        return casca.reply('You need to provide some input.')
    bits = text.split(',')
    master_url = False
    if len(bits) == 2:
        ## county given
        ...
        for line in page1:
            ...
            prev1 = line
        if not url_part2:
            return casca.reply('Could not find county.')
        ...
        if bits[0]:
            ...
            if not zips:
                return casca.reply('ZIP is invalid.')
            else:
                try:
                    zip_code = zips[0][0]
                except:
                    return casca.reply('ZIP could not be validated.')
            ...
            if fips:
                ...
                if not state and not city:
                    return casca.reply('Could not match ZIP code to a state')
                try:
                    state = state[0].lower()
                    ...
                    return casca.reply('Could not parse state or city from database.')
            else:
                return casca.reply('ZIP code does not exist.')

    if not master_url:
        return casca.reply('Invalid input. Please enter a ZIP code or a county and state pairing, such as \'Franklin, Ohio\'')

    feed = feedparser.parse(master_url)
    warnings_dict = dict()
    for item in feed.entries:
        if nomsg[:51] == colourize(item['title']):
            return casca.say(nomsg.format(location))
        else:
    ...
    if len(warnings_dict) > 0:
        ...
        if input.sender.startswith('#') and not (input.group(1)).startswith('nws-more'):
            ...
            for key in warn_list_dt:
                casca.say(key)
            casca.say(more_info.format(location, master_url))
        else:
            ## if queried in private message
            for key in warn_list_dt:
                casca.say(key)
                casca.say(warnings_dict[key])
            casca.say(more_info.format(location, master_url))
...
def warns_control(casca, input):
    global stop
    if not input.admin:
        return casca.reply('You need to be an admin to use this!')

    if input.group(2) == 'start':
        stop = False
        casca.reply('Starting...')
        weather_feed(casca)
    elif input.group(2) == 'stop':
        stop = True
        casca.reply('Stopping...')
warns_control.commands = ['warns']
warns_control.priority = 'high'
warns_control.thread = True


def weather_feed(casca):
    global stop
    ...
    if not stop:
        while True:
            if stop:
                ...
                c.close()
                stop = False
                return casca.reply('Checking... I\'m comleteply stopped. (stopped in "Master")')
            ...
            for entity in all_entries:
                if stop:
                    ...
                    c.close()
                    stop = False
                    return casca.reply('Checking... I\'m completely stopped. (stopped inside XML Parse)')
                entry = entity
                try:
                    ...
                    casca.msg(casca.logchan_pm, 'No entry available. See stdout for more information.')
                ...
                if len(c.fetchall()) < 1:
                    ...
                    counter_areas = 1
                    for each in areas:
                        casca.msg(ch_state, line1.format(each, state, str(counter_areas).zfill(2), str(len_areas).zfill(2)))
                        counter_areas += 1

                    casca.msg(ch_state, line2.format(title, cert, severity, status, urgency))

                    summaries = textwrap.wrap(summary, 450)
                    len_summaries = len(summaries)
                    counter_summaries = 1
                    for each in summaries:
                        casca.msg(ch_state, 'Part %s of %s: %s' % (str(counter_summaries).zfill(2), str(len_summaries).zfill(2), each))
                        counter_summaries += 1
