#!/usr/bin/env python
"""
countdown.py - casca Countdown Module
...
def generic_countdown(casca, input):
    ...
    if text and len(text.split()) >= 3:
        ...
        if not year.isdigit() and not month.isdigit() and not day.isdigit():
            return casca.reply('What are you even trying to do?')
    ...
    if text and len(text) >= 3 and year.isdigit() and month.isdigit() and day.isdigit():
        ...
        if abs(float(offset)) >= 14:
            return casca.reply('Do you not love me anymore?')
