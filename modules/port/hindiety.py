...
# -*- coding: utf-8 -*-
"""
hindiety.py - casca Hindi Etymology Module
...
def hindiety(casca, word):
    parts = word.split(' ')

    if len(parts) == 1:
        casca.say('No term given.')
        return
    ...
    if index == -1:
        casca.say('Term `%s\' not found.' % term)
        return
...
def init(casca, bytes):
    ...
    if output == []:
        casca.say('Speech `%s\' not found for term `%s\'' % \
                (speech, term))
        return
