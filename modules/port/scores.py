...
class Scores:
    def __init__(self):
        self.scores_filename = os.path.expanduser('~/.casca/scores.txt')
    ...
    def editpoints(self, casca, input, nick, points):
        if not nick:
            return
        nick = uc.encode(nick.lower())
        if not nick:
            casca.reply(self.STRINGS["cantadd"])
        elif (not input.admin) and (input.nick).lower() == nick:
            casca.reply(self.STRINGS["denied"])
        else:
            nick = nick.lower()
            ...
            self.save()
            casca.say(self.str_score(nick, chan))
    ...
    def view_scores(self, casca, input):
        ...
        if len(line) == 0 or (len(line) == 1 and line[0] == 'top'):
            ...
            ## .scores <nick>
            casca.say(given_user(line[0], current_channel))
            ...
            ## .scores <channel> <nick>
            casca.say(given_user(line[1], line[0]))
    ...
    def setpoint(self, casca, input, line):
        if not input.admin:
            return
        line = line[10:].split()
        if len(line) != 4:
        ...
        try:
            ...
            casca.say(self.STRINGS["invalid"])
            return

        if add < 0 or sub < 0:
            casca.reply("You are doing it wrong.")
            return
        ...
        self.save()
        casca.say(self.str_score(nick, channel))

    def rmuser(self, casca, input, line):
        if not input.admin:
            return
        if len(line) < 9:
            casca.reply("No input provided.")
            return
        line = line[8:].split()
