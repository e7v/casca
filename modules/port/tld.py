...
def gettld(casca, input):
    '''.tld .sh -- displays information about a given top level domain.'''
    global soup

    text = input.group(2)
    if not text:
        casca.reply("You didn't provide any input.")
        return
    text = text.split()[0]
    ...
    for tld in tlds:
        ...
        if len(tld_tds[0]('a')) > 0 and tld_tds[0]('a')[0].text == text:
            ...
            return casca.say('Entity: %s (Explanation: %s, Notes: %s). IDN: %s, DNSSEC: %s, SLD: %s, IPv6: %s' % (new_out['entity'], new_out['expl'], new_out['notes'], new_out['idn'], new_out['dnssec'], new_out['sld'], new_out['ipv6']))

    return casca.say('No matches found for TLD: %s' % (text))
...
def tld_cache(casca, input):
    casca.say('TLD cache from Wikipedia last updated: ' + str(last_updated))

    reload_cache()

    casca.say('TLD cache is now current.')
