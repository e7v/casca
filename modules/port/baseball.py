...
def mlb(casca, input):
    ...
    txt = input.group(2)
    if not txt:
        return casca.reply('No input provided.')
    ...
    if (txt_list[0]).lower() == 'pbp':
        ...
        if info['team_game']:
            if 'pbp' in info['team_game']:
                ...
                if pbp:
                    return casca.reply(pbp)
        return casca.reply('Could not find play by play.')
    else:
        ...
        if team_game and 'alerts' in team_game:
            ...
            casca.reply('No information can be ascertained.')
mlb.commands = ['mlb']
mlb.rate = 20
