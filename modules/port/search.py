#!/usr/bin/env python
"""
search.py - casca Web Search Module
...
def duck(casca, input):
    ...
    query = input.group(2)
    if not query:
        return casca.reply('.ddg what?')

    #query = query.encode('utf-8')
    #casca.say('query: ' + query)
    ...
    if result and len(result) == 1:
        if hasattr(casca, 'last_seen_uri') and input.sender in casca.last_seen_uri:
            casca.last_seen_uri[input.sender] = result[0]
    ...
    if not uri and (not result or not len(result) >= 1):
        return casca.reply("No results found for '%s'." % query)
