#!/usr/bin/env python
"""
twitter.py - casca Twitter Module
...
def initialize_keys(casca, consumer_key='', consumer_secret=''):
    global client
    if not hasattr(casca.config, 'twitter_consumer_key') or\
           not hasattr(casca.config, 'twitter_consumer_secret'):
               return False, "Please sign up for Twitter's API and provide the keys in the configuration."
    consumer_key = casca.config.twitter_consumer_key
    consumer_secret = casca.config.twitter_consumer_secret
...
def twitter(casca, input):
    status, response = initialize_keys(casca)
    if not status:
        return casca.say(response)

    arg = input.group(2)

    if not arg:
        if hasattr(casca, 'last_seen_uri') and input.sender in casca.last_seen_uri:
            temp = casca.last_seen_uri[input.sender]
            if '//twitter.com' in temp:
                arg = temp
            else:
                return casca.say('Last link seen in this channel is not from twitter.com.')
        else:
            return casca.reply('Give me a link, a username, or a tweet id.')
