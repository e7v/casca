...
def manage_rss(casca, input):
    """.rss-admin operation channel site_name url -- 'add', 'del', or 'list' rss"""
    if not input.admin:
        casca.reply("Sorry, you need to be an admin to modify the RSS feeds.")
        return
    ...
    if len(text) < 2:
        casca.reply("Proper usage: '.rss-admin add ##channel Site_Name URL', '.rss-admin del ##channel Site_Name URL', '.rss-admin del ##channel'")
    elif len(text) > 2:
    ...
    if len(text) > 4 and text[1] == 'add':
        ...
        if len(temp) == 1:
            ...
            casca.reply("Not enough parameters specified.")
            return
        if fg_colour:
        ...
        conn.commit()
        c.close()
        casca.reply("Successfully added values to database.")
    elif len(text) == 3 and text[1] == 'del':
        # .rss-admin del ##channel
        c.execute("DELETE FROM rss WHERE channel = ?", (channel,))
        conn.commit()
        c.close()
        casca.reply("Successfully removed values from database.")
        ...
        conn.commit()
        c.close()
        casca.reply("Successfully removed the site from the given channel.")
    elif len(text) == 2 and text[1] == 'list':
        c.execute("SELECT * FROM rss")
        k = 0
        for row in c:
            k += 1
            casca.say("list: " + unicode(row))
        if k == 0:
            casca.reply("No entries in database")
    else:
        casca.reply("Incorrect parameters specified.")
    c.close()
...
def read_feeds(casca):
    ...
    if not c.fetchall():
        STOP = True
        casca.say("No RSS feeds found in database. Please add some rss feeds.")
    ...
    for row in c:
        ...
        try:
            ...
            casca.say("Can't parse.")

        try:
            entry = fp.entries[0]
        except:
            if DEBUG:
                casca.say("row: " + str(row))
                casca.say("Can't find element: " + str(fp))
            continue
        ...
        if len(cursor_recent.fetchall()) < 1:
            ...
            casca.msg(feed_channel, response)
            ...
            if DEBUG:
                if hasattr(casca.config, 'logchan_pm'):
                    casca.msg(casca.config.logchan_pm, u"Skipping previously read entry for %s: %s %s" % (feed_channel, site_name_effect, entry.title))
                else:
                    casca.say(u"Skipping previously read entry for %s: %s %s" % (feed_channel, site_name_effect, entry.title))
    cursor_recent.close()
    c.close()


def startrss(casca, input):
    """Begin reading RSS feeds"""
    if not input.admin:
        casca.reply("You must be an admin to start up the RSS feeds.")
        return
    ...
    if query == '-v':
        DEBUG = True
        STOP = False
        casca.reply("Debugging enabled.")
    elif query == '-q':
        DEBUG = False
        STOP = False
        casca.reply("Debugging disabled.")
    elif query == '-i':
        INTERVAL = input.group(3)
        casca.reply("INTERVAL updated to: %s" % (str(INTERVAL)))
    elif query == 'stop':
        STOP = True
        casca.reply("Stop parameter updated.")
    elif query == 'start':
        STOP = False
    else:
        return casca.reply("We couldn't figure out what you wanted to do.")


    if first_run:
        if DEBUG:
            casca.say("Okay, I'll start rss fetching...")
        first_run = False
    else:
        restarted = True
        if DEBUG:
            casca.say("Okay, I'll re-start rss...")

    if not STOP:
        casca.say('Starting rss...')
        while True:
            if STOP:
                casca.reply("STOPPED")
                first_run = False
                ...
                break
            if DEBUG:
                casca.say("Rechecking feeds")
            read_feeds(casca)
            time.sleep(INTERVAL)

    if DEBUG:
        casca.say("Stopped checking")
