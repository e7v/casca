...
# Place a file in your ~/casca/ folder named, bitly.txt
...
BITLY_TRIGGER_LEN_TITLE = None
BITLY_TRIGGER_LEN_NOTITLE = None
EXCLUSION_CHAR = '!'
IGNORE = list()
...
def noteuri(casca, input):
    ...
    if not hasattr(casca, 'last_seen_uri'):
        casca.last_seen_uri = {}
    casca.last_seen_uri[input.sender] = uri
...
def generateBitLy(casca, input):
    url = input.group(2)
    if not url:
        if hasattr(casca, 'last_seen_uri') and input.sender in casca.last_seen_uri:
            url = casca.last_seen_uri[input.sender]
        else:
            return casca.say('No URL provided')

    bitly = short(url)
    for b in bitly:
        displayBitLy(casca, b[0], b[1])
generateBitLy.commands = ['bitly']
generateBitLy.priority = 'high'


def displayBitLy(casca, url, shorten):
    if url is None or shorten is None:
        return
    u = getTLD(url)
    shorten = shorten.replace('http:', 'https:')
    casca.say('%s  -  %s' % (u, shorten))
...
def show_title_auto(casca, input):
    ...
    if hasattr(casca.config, 'auto_title_disable_chans'):
        disabled_channels = casca.config.auto_title_disable_chans
...
def show_title_demand(casca, input):
    ...
    if not uri:
        ...
        if not hasattr(casca, 'last_seen_uri'):
            casca.last_seen_uri = dict()
        if channel in casca.last_seen_uri:
            uri = casca.last_seen_uri[channel]
        else:
            return casca.say('No recent links seen in this channel.')
...
def collect_links(casca, input):
    link = input.groups()
    channel = input.sender
    link = link[0]
    if not hasattr(casca, 'last_seen_uri'):
        casca.last_seen_uri = dict()
    casca.last_seen_uri[channel] = link
...
def unbitly(casca, input):
    ...
    if new_url:
        ...
        try:
            ...
            return casca.say('Failed to grab URL: %s' % (url))

    channel = input.sender
    if not hasattr(casca, 'last_seen_uri'):
        casca.last_seen_uri = dict()
    casca.last_seen_uri[channel] = new_url

    if new_url.startswith(('http://', 'https://')):
        casca.say(new_url)
    else:
        casca.say('Failed to obtain final destination.')
unbitly.commands = ['unbitly', 'untiny', 'longurl', 'st', 'short']
unbitly.priority = 'low'
unbitly.example = '.unbitly http://git.io/6fY4OQ'


def puny(casca, input):
    '''.puny -- convert to xn-- code for URLs'''
    text = input.group(2)
    if not text:
        return casca.say('No input provided.')
    ...
    if text.startswith('xn--'):
        ...
        try:
            ...
            return casca.say('Stop being a twat.')
