...
"""
weather.py - casca Weather Module
...
def show_metar(casca, input):
    '''.metar <location> -- shows the raw METAR data for a given location'''
    txt = input.group(2)

    if not txt:
        return casca.say('Try .metar London, for example?')

    status, icao_code = get_icao(casca, txt, 'metar')
    if not status:
        return casca.say(icao_code)

    status, metar = get_metar(icao_code)
    if not status:
        return casca.say(metar)
...
def f_weather(casca, input):
    ...
    status, icao_code = get_icao(casca, text)
    print('status:{}'.format(status))
    print('icao_code:{}'.format(icao_code))
    if not status:
        return casca.say(icao_code)
    ...
    if metar[0] == 'AUTO':
        metar = metar[1:]
    if metar[0] == 'VCU':
        casca.say(icao_code + ': no data provided')
        return
    ...
    if not metar:
        return casca.say(icao_code + ': no data provided')
...
def windchill(casca, input):
    ...
    if len(text) == 1:
        ...
        return casca.say(u'.windchill <temp> <wind speed> -- shows Windchill in \u00B0F')
    ...
    if len(text) >= 3:
        try:
            ...
            return casca.say('Invalid arguments! Try, .windchill without any parameters.')
    else:
        ## why couldn't the input be valid? silly users
        return casca.say('Invalid arguments! Try, .windchill without any parameters.')

    if temp > 50:
        return casca.say(u'The windchill formula only works on temperatures below 50 \u00B0F')

    if wind < 0:
        ## yes, yes, I know about vectors, but this is a scalar equation
        return casca.say("You can't have negative wind speed!")
    elif wind >= 300:
        ## hehe
        casca.reply('Are you okay?')

    ## cf. https://is.gd/mgLuzU
    wc = 35.74 + (0.6215 * temp) - (35.75 * (wind ** (0.16))) + (0.4275 * temp * (wind ** (0.16)))

    casca.say(u'Windchill: %2.f \u00B0F' % (wc))
...
def forecast(casca, input):
    if not hasattr(casca.config, 'forecastio_apikey'):
        return casca.say('Please sign up for a Dark Sky API key at https://darksky.net/ or try .wx-noaa or .weather-noaa')

    txt = input.group(2)
    if not txt:
        return casca.say('Please provide a location.')
    ...
    url = url % (casca.config.forecastio_apikey, urllib.parse.quote(lat), urllib.parse.quote(lng))
    ...
    try:
        ...
        return casca.say('Could not acess https://api.darksky.net/')

    ## we want some tasty JSON
    try:
        data = json.loads(page)
    except:
        return casca.say('The server did not return anything that was readable as JSON.')

    if 'daily' not in data or 'data' not in data['daily']:
        return casca.say('Cannot find usable info in information returned by the server.')
    ...
    if 'alerts' in data:
        ...
        for alert in data['alerts']:
            casca.say(alert['title'] + ' Expires at: ' + str(datetime.datetime.fromtimestamp(int(alert['expires']))))
...
def forecastio_current_weather(casca, input):
    if not hasattr(casca.config, 'forecastio_apikey'):
        return casca.say('Please sign up for a darksky.net API key at https://darksky.net/')
    ...
    if 'currently' not in data:
        ...
        return casca.say('No information obtained from darksky.net for the given location: %s,' % (name, lat, lng,) )
...
def weather_wunderground(casca, input):
    if not hasattr(casca.config, 'wunderground_apikey'):
        return casca.say('Please sign up for a wunderground.com API key at https://www.wunderground.com/weather/api/ or try .wx-noaa or .weather-noaa')

    apikey = casca.config.wunderground_apikey

    url = 'https://api.wunderground.com/api/%s/conditions/geolookup/q/%s.json'
    txt = input.group(2)
    if not txt:
        return casca.say('No input provided. Please provide a locaiton.')
    ...
    try:
        ...
        return casca.say("We could not access wunderground.com's API at the moment.")
    ...
    try:
        ...
        return casca.say('We could not obtain useful information from the wunderground.com API.')

    if 'response' in useful and 'error' in useful['response'] and 'description' in useful['response']['error']:
        return casca.say(str(useful['response']['error']['description']))

    if 'current_observation' not in useful:
        return casca.say('No observations currently found.')
...
def forecast_wg(casca, input):
    ...
    if 'forecast' not in useful:
        return casca.say('We could not find a forecast for the given location.')
