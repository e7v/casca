...
USER_AGENT = "Casca/1.0 (https://github.com/e7v/casca) Casca/1.0"

class JenniURLopener(FancyURLopener):
    version = USER_AGENT
...
def random_quote(casca, cat):
    if cat is not None:
        if cat not in CATS:
            casca.say("I don't know that category, please select from one of: {0}".format(', '.join(CATS)))
            return
    else:
    ...
    while(True):
        ...
        cat_url_content = urlopen(cat_url).read()
        content = json.loads(cat_url_content.decode())
        cat_members = content["query"]["categorymembers"]

        # Select at random
        random_member = choose_random_member(cat_members)
        if random_member is None:
            casca.say("An error occurred fetching a subcategory")
            return
    ...
    try:
        ...
        content = json.loads(urlopen(page_url).read().decode())
        ...
        while quote == None and num_tries < MAX_TRIES:
            ...
            if section is None:
                casca.say("We accidentally chose a page with no quotes, sorry about that!")
                return
    
            section_index = randrange(len(sections)) + 1
    
            section_url = BASE_URL + SECTION % (page_id, section_index)
            content = json.loads(urlopen(section_url).read().decode())
            section_title = content["parse"]["title"]
            html = Soup(content["parse"]["text"]["*"], 'html5lib')
            all_quotes = []
        ...
        if quote is None:
          casca.say("We accidentally chose a section of a page with no quotes, sorry about that!")
          return

        casca.say("{0}: {1}".format(section_title, quote.encode('utf-8')).replace('&#160;', ' '))
    except Exception as e:
        casca.say("An error occurred fetching a quote: {0}".format(e))
        return

def wikiquote(casca, input):
    ...
    random_quote(casca, origterm)
wikiquote.commands = ['wq', 'wikiquote']
wikiquote.priority = 'low'
wikiquote.rate = 10

def list_cats(casca, input):
    casca.say("WikiQuote categories I know: {0}".format(", ".join(CATS)))
